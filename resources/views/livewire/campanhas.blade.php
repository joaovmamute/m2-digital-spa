<div class="mt-6">
    ...
    <div class="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
        <div class="inline-block min-w-full overflow-hidden rounded-lg shadow">
                ...
                <tbody>
                    @foreach ($this->campanhas as $campanha)
                        <tr>
                            ...
                            <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                                <p class="text-gray-900 whitespace-nowrap">
                                    {{ number_format(bcmul($campanha->desconto, '100', 2), 2, ',') . ' %' }}
                                </p>
                            </td>
                            <td class="px-5 py-5 text-sm bg-white border-b border-gray-200 w-60 text-right">
                                {{-- <span wire:click="openCampanhaDetalhe({{ $campanha->id }})" class="relative inline-block px-3 py-1 font-semibold text-blue-900 leading-tight cursor-pointer">
                                ...
                                <span wire:click="openProdutosModal({{ $campanha->id }})" class="relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight cursor-pointer">
                                ...
                                <span wire:click="openModal({{ $campanha->id }})" class="relative inline-block px-3 py-1 font-semibold text-yellow-900 leading-tight cursor-pointer">
                                ...
                                <span wire:click="openDeleteModal({{ $campanha->id }})" class="relative inline-block px-3 py-1 font-semibold text-red-900 leading-tight cursor-pointer">
            ...
            {{ $this->campanhas->links('vendor.livewire.simple-tailwind') }}
        </div>
    </div>
    ...
    <x-jet-dialog-modal wire:model="modal">
        <x-slot name="title">
            <div class="flex justify-between">
                @if (!$campanha_id)
                    <p class="text-2xl font-bold">{{ __('Criar Campanha') }}</p>
                @else
                    <p class="text-2xl font-bold">{{ __('Editar Campanha') }}</p>
                @endif
        ...
        <x-slot name="content">
            ...
            <div>
                <h2 class="text-xl font-semibold leading-tight text-gray-700">
                    {{ __('Desconto (ex: 10.00)') }}
                </h2>
                <input wire:model="desconto" class="w-full mt-2 border-gray-200 rounded-md focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500" type="number" min="0" max="100" step=".01">
                @error('desconto')
                    <span class="error" style="color: #ff0000">{{ $message }}</span>
                @enderror
            </div>
    ...
    <x-jet-dialog-modal wire:model="deleteModal">
        <x-slot name="title">
            <div class="flex justify-between">
                <p class="text-2xl font-bold">{{ __('Deletar Campanha') }}</p>
        ...
        <x-slot name="content">
            <h2 class="text-xl font-semibold leading-tight text-gray-700">
                {{ __('Deseja deletar a campanha ') . $nome . '?' }}
            </h2>
        </x-slot>
    ...
    <x-jet-dialog-modal wire:model="produtosModal">
        <x-slot name="title">
            <div class="flex justify-between">
                <p class="text-2xl font-bold">{{ __('Editar Produtos da ') . $nome }}</p>

                <div wire:click="$toggle('produtosModal')" class="z-50 cursor-pointer modal-close">
                    <svg class="text-black fill-current" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18">
                        <path d="M14.53 4.53l-1.06-1.06L9 7.94 4.53 3.47 3.47 4.53 7.94 9l-4.47 4.47 1.06 1.06L9 10.06l4.47 4.47 1.06-1.06L10.06 9z"></path>
                    </svg>
                </div>
        ...
        <x-slot name="content">
            ...
            <div class="grid grid-cols-3 gap-2">
                @foreach ($this->produtos as $produto)
                    <div wire:click="analisaProduto({{ $produto->id }})" class="px-3 py-2 ml-0 leading-tight border border-gray-200 rounded-l hover:bg-indigo-400 hover:text-white cursor-pointer text-center @if($produto->campanhas->find($campanha_id)) text-white bg-indigo-500 @else text-indigo-700 bg-white @endif">
                        <span>
                            {{ $produto->nome }}
                        </span>
                    </div>
        ...
        <x-slot name="footer">
            <div class="flex justify-end pt-2">
                <button wire:click="$toggle('produtosModal')" class="px-6 py-3 font-medium tracking-wide text-white bg-indigo-600 rounded-md hover:bg-indigo-500 focus:outline-none">
                    {{ __('Fechar') }}
                </button>
            </div>
        </x-slot>
