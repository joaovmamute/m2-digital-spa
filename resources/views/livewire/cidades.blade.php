<div class="mt-6">
    ...
    <div class="flex flex-col mt-3 sm:flex-row justify-between">
        <div class="flex">
            <div class="relative">
                <select wire:model="filtroGrupo" class="block w-full h-full px-4 py-2 pr-8 leading-tight text-gray-700 bg-white border-t border-b border-r border-gray-400 rounded-r appearance-none sm:rounded-r-none sm:border-r-0 focus:outline-none focus:border-l focus:border-r focus:bg-white focus:border-gray-500">
                    <option value="">{{ __('Grupos') }}</option>
    ...
    <div class="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
        <div class="inline-block min-w-full overflow-hidden rounded-lg shadow">
                    ...
                    @foreach ($this->cidades as $cidade)
                        <tr>
                            ...
                            <td class="px-5 py-5 text-sm bg-white border-b border-gray-200 w-60 text-right">
                                <span wire:click="openModal({{ $cidade->id }})" class="relative inline-block px-3 py-1 font-semibold text-yellow-900 leading-tight cursor-pointer">
                                ...
                                <span wire:click="openDeleteModal({{ $cidade->id }})" class="relative inline-block px-3 py-1 font-semibold text-red-900 leading-tight cursor-pointer">
            ...
            {{ $this->cidades->links('vendor.livewire.simple-tailwind') }}
        </div>
    </div>
    ...
    <x-jet-dialog-modal wire:model="modal">
        <x-slot name="title">
            <div class="flex justify-between">
                @if (!$cidade_id)
                    <p class="text-2xl font-bold">{{ __('Criar Cidade') }}</p>
                @else
                    <p class="text-2xl font-bold">{{ __('Editar Cidade') }}</p>
                @endif
        ...
        <x-slot name="content">
            ...
            <div>
                ...
                <div class="relative">
                    <select wire:model="grupo_id" class="block w-full h-full mt-2 px-4 py-2 pr-8 leading-tight text-gray-700 bg-white border border-gray-400 rounded-l appearance-none focus:outline-none focus:bg-white focus:border-gray-500">
                        @if ($this->grupos->count())
                            <option value="{{ null }}">{{ __('Selecione um grupo...') }}</option>
                            @foreach ($this->grupos as $grupo)
                                <option value="{{ $grupo->id }}">{{ $grupo->nome }}</option>
                            @endforeach
                        @else
                            <option value="{{ null }}">{{ __('Nenhum Grupo cadastrado.') }}</option>
                        @endif
                    </select>
                    <div class="absolute inset-y-0 right-0 flex items-center px-2 text-gray-700 pointer-events-none">
                        <svg class="w-4 h-4 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
    ...
    <x-jet-dialog-modal wire:model="deleteModal">
        <x-slot name="title">
            <div class="flex justify-between">
                <p class="text-2xl font-bold">{{ __('Deletar Cidade') }}</p>
        ...
        <x-slot name="content">
            <h2 class="text-xl font-semibold leading-tight text-gray-700">
                {{ __('Deseja deletar a cidade ') . $nome . '?' }}
            </h2>
        </x-slot>
